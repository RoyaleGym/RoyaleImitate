...
from royalelearn.extensions import PreflightError, digest_of
...
if TYPE_CHECKING:  # pragma: no cover - annotations only
    ...
    from royalelearn.extensions import EnvSpec, ObsBatch, RowCodec, RunConfig
...
class ShardContext(msgspec.Struct, frozen=True, kw_only=True):
    ...
    def from_parts(
        ...
        from royalelearn.extensions import engine_build
    ...
    def of_config(cls, config: RunConfig) -> ShardContext:
        ...
        from royalelearn.extensions import action_digest_of, build_env, read_env_spec
...
def _packages() -> dict[str, str]:
    """The code that wrote the shards: RoyaleLearn, RoyaleGym and this package, each by version
    and commit."""
    ...
    own = package_provenance(royaleimitate)
    return {
        "royalelearn": f"{royalelearn_version()} ({git_describe()})",
        "royalegym": f"{gym_version} ({gym_git})",
        "royaleimitate": f"{royaleimitate.__version__} ({own})",
    }
