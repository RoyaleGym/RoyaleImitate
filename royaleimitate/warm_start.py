...
from royalelearn.extensions import ExtensionBase, RunContext

from .config import WarmStartSection, warm_start_problems

if TYPE_CHECKING:  # pragma: no cover - annotations only
    from royalelearn.extensions import Alarm, RunConfig, ScheduleSpec, SchemaContribution
...
class WarmStart(ExtensionBase):
    ...
    def alarms(self, section: WarmStartSection) -> Sequence[Alarm]:
        if section.actor_lr_scale is None:
            return ()
        from royalelearn.extensions import freeze_alarms
    ...
    def metric_schema(self, section: WarmStartSection) -> SchemaContribution:
        from royalelearn.extensions import FREEZE_ALARM_KEYS, SchemaContribution
